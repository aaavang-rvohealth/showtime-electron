...
export const useSelectDanceModal = (): [ReturnType<typeof useDisclosure>, (props: SelectModalHookProps) => React.ReactNode] => {
  ...
  return [selectDanceModal, ({ onSubmit }: SelectModalHookProps) => <SelectDanceModal onSubmit={onSubmit}
                                                                                      disclosure={selectDanceModal} />];
};
...
const SelectDanceModal: React.FC<SelectDanceModalProps> = ({
  ...
  const [selectedHydratedDanceVariant, setSelectedHydratedDanceVariant] = useState({} as Partial<HydratedDanceVariant>);
  ...
  const loadDanceVariantOptions = async (inputValue: string): Promise<Option<DanceVariant>[]> => {
    ...
    if(inputValue.length === 0) {
      return danceVariants.map((danceVariant) => ({ value: danceVariant, label: danceVariant.title }));
    } else {
      return danceVariants.filter(danceVariant => danceVariant.title.toLowerCase().includes(inputValue.toLowerCase())).map((danceVariant) => ({ value: danceVariant, label: danceVariant.title }));
    }
  };
        ...
        <ModalBody>
          <VStack gap={'5px'} width={'100%'}>
            <>
              ...
              }} cacheOptions defaultOptions loadOptions={loadDanceOptions} onChange={async (option: any) => {
                const defaultDanceVariant = await database.danceVariants.where('danceId')
                  .equals(option.value.id)
                  .and(v => v.defaultVariant)
                  .first();

                const song = await database.songs.get(defaultDanceVariant!.songId);
                ...
                <AsyncSelect styles={{
                  ...
                  option: (baseStyles, state) => ({
                    ...
                    color: 'black'
                             ...
                             cacheOptions defaultOptions defaultValue={selectedHydratedDanceVariant.danceVariant}
                             defaultInputValue={selectedHydratedDanceVariant.danceVariant?.title}
                             loadOptions={loadDanceVariantOptions} onChange={async (option: any) => {
